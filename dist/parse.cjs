...
function epub_default(content) {
  return __async(this, null, function* () {
    ...
    let chapters_lookup = opf.package.spine.itemref.filter(
      (obj) => ![
        "coverPage",
        "chapter1",
        "chapter2"
    ...
    for (const lookup of chapters_lookup) {
      const chapter_item_node = opf.package.manifest.item.find((node) => node["@_id"] === lookup);
      ...
      if (typeof chapterContents[0] === "string" && !chapterContents[0].trim())
        chapterContents.shift();
      for (let i = chapterContents.length - 1; i >= 0; i--) {
        const content2 = chapterContents[i];
        if (typeof content2 !== "string" || typeof content2 === "string" && content2.trim())
          break;
        chapterContents.pop();
      }
      story.Content.push(chapter);
...
function main() {
  return __async(this, null, function* () {
    if (!import_fs.default.existsSync(test_file))
      return;
    const test_file_path = import_path.default.dirname(test_file);
    const test_file_extension = import_path.default.extname(test_file);
    const test_file_name = import_path.default.basename(test_file, test_file_extension);
    const test_output_path = import_path.default.join(test_file_path, test_file_name);
    const story = yield src_default(import_fs.default.readFileSync(test_file, test_file_extension === ".epub" ? null : "utf-8"));
    import_fs.default.writeFileSync(test_output_path + ".json", JSON.stringify(story, null, "	"));
  });
}
